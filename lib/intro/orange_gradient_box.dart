import 'package:flutter/material.dart';
import 'package:flutter_sdg/intro/lime_green_gradient_box.dart';
import 'package:flutter_sdg/layout/intro_layout.dart';
import '../core/navigation/route_transitions.dart';

class OrangeGradientBoxScreen extends StatelessWidget {
  const OrangeGradientBoxScreen({super.key});

  @override
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: () {
        Navigator.push(
          context,
          RouteTransitions.createSlideTransitionRoute(
              const LimeGreenGradientBoxScreen()),
        );
      },
      child: IntroLayout(
        body: OrangeGradientBox(),
      ),
    );
  }
}

class OrangeGradientBox extends StatelessWidget {
  OrangeGradientBox({super.key});

  @override
  Widget build(BuildContext context) {
    return Container(
      height: MediaQuery.of(context).size.height / 2.5,
      decoration: BoxDecoration(
        gradient: LinearGradient(
          begin: Alignment.bottomCenter,
          end: Alignment.topCenter,
          colors: _gradientColors,
          stops: _gradientStops,
        ),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          _buildGreetingText(),
          _buildDescriptionText(),
          _buildGetStartedText(),
        ],
      ),
    );
  }

  final List<Color> _gradientColors = [
    const Color(0xffFD6925).withOpacity(1.0),
    const Color(0xffFD6925).withOpacity(1.0),
    const Color(0xffFD6925).withOpacity(0.0),
  ];

  final List<double> _gradientStops = [0.0, 0.5, 1.0];

  Widget _buildGreetingText() {
    return const Padding(
      padding: EdgeInsets.only(top: 40, bottom: 40, left: 20, right: 20),
      child: Text(
        'Group Challenges',
        style: TextStyle(
          fontFamily: 'OswaldLight',
          color: Colors.white,
          fontSize: 21,
        ),
      ),
    );
  }

  Widget _buildDescriptionText() {
    return const Padding(
      padding: EdgeInsets.only(bottom: 40, left: 20, right: 20),
      child: Text(
        'Zero dolor sit amet, consectetur adipiscing elit, sed do eiusmod. dolor sit amet, con',
        style: TextStyle(
          fontFamily: 'OswaldLight',
          color: Colors.white,
          fontSize: 19,
        ),
      ),
    );
  }

  Widget _buildGetStartedText() {
    return const Center(
      child: Text(
        'Alright',
        style: TextStyle(
          fontFamily: 'OswaldLight',
          color: Colors.white,
          fontSize: 30,
        ),
      ),
    );
  }
}
